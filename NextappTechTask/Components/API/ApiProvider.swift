...
public class ApiProvider {
    ...
    private let requestQueue = DispatchQueue(label: "com.alza.response-queue",
                                             qos: .background,
                                             attributes: [DispatchQueue.Attributes.concurrent])
    private let logQueue = DispatchQueue(label: "com.alza.api-log", qos: .background)
                    ...
                    do {
                        ...
                        case .success(let data):
                            if let status = dataResponse.response?.statusCode,
                                ...
                                debugString = "DEBUG: - ApiProvider: deserealization of \(String(describing: dataResponse.request?.url?.absoluteString)) to \(ErrorEntity.self) success"
                                self.log(message: debugString)
                                debugString = "DEBUG: - ApiProvider: decoded string of \(T.self): \(String(describing: String(data: data, encoding: .utf8))) success"
                                self.log(message: debugString)
                                observer.onNext(ApiEvent.error(error))
                            } else {
                                let item = try T.decode(data: data)
                                debugString = "DEBUG: - ApiProvider: deserealization of \(String(describing: dataResponse.request?.url?.absoluteString)) to \(T.self) success"
                                self.log(message: debugString)
                                debugString = "DEBUG: - ApiProvider: decoded string of \(T.self): \(String(describing: String(data: data, encoding: .utf8))) success"
                                self.log(message: debugString)
                        ...
                        case .failure(let error):
                            debugString = "DEBUG: - ApiProvider: loading of \(String(describing: dataResponse.request?.url?.absoluteString)) fail. With status code: \(String(describing: dataResponse.response?.statusCode)). Object: \(T.self) Error: \(error)"
                            self.log(message: debugString)
                            observer.onNext(ApiEvent.error(error))
                        }
                    } catch {
                        debugString = "DEBUG: - ApiProvider: deserealization of \(String(describing: dataResponse.request?.url?.absoluteString)) fail. With status code: \(String(describing: dataResponse.response?.statusCode)). Object: \(T.self) Error: \(error)"
                        observer.onNext(ApiEvent.error(error))
                        self.log(message: debugString)
                    }
                })
    ...
    public func createRequest(
        ...
        let debugString = "DEBUG: - ApiProvider: request to \(url), method \(method), params: \(String(describing: parameters))"
        self.log(message: debugString)
